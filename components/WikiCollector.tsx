...
import { fetchWikiPage, parseWikiUrl, WikiPageData, WikiMenuItem, generateAutoCategorizedTree } from '../utils/wikiCrawler';
...
export const WikiCollector: React.FC<WikiCollectorProps> = ({
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const consolidatedLinks = Array.from(new Set(successPages.flatMap(p => p.links)));

      // Generate a single unified menuTree using our union of links and the base domain
      const mergedMenuTree = generateAutoCategorizedTree(consolidatedLinks, domain);
  ...
  const handleStartCrawling = async () => {
    ...
    // Giảm concurrency xuống 2 để tránh bị server wiki chặn/rate-limit
    const CONCURRENCY_PASS1 = 2;
    const CONCURRENCY_PASS2 = 1; // Pass 2 chạy tuần tự 1 task/lần cho an toàn tuyệt đối
