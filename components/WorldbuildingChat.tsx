  ...
  const runPipeline = async (attachedDoc: { name: string; content: string }) => {
    ...
    try {
      for (let i = 0; i < enabledSteps.length; i++) {
        ...
        let stepDone = false;
        let stepCreatedCount = 0;
        let chunkIndex = 0;
        let consecutiveErrors = 0;
        let consecutiveEmptyDones = 0; // Anti-skip: đếm số lần AI trả DONE mà không tạo entry nào
        const MAX_EMPTY_DONE_RETRIES = 3; // Tối đa 3 lần retry trước khi chấp nhận
        const chunkSize = 15000;

        // Thu thập thông tin các bước đã hoàn thành trước đó làm bối cảnh loại trừ lặp lại (Anti-repetition metadata)
        const prevStepsMeta = enabledSteps.slice(0, i).map((s, idx) => `Bước ${idx + 1}: ${s.name}`).join(', ');
        
        // Thu thập tên các entry đã tạo ở các bước trước (để kiểm tra cross-step contamination)
        const entriesBeforeStep = currentLorebookState.entries.map(e => e.comment?.toLowerCase()).filter(Boolean);

        while (!stepDone && pipelineRef.current) {
          setPipelineSubLoading(true);

          let currentInput = '';
          const start = chunkIndex * chunkSize;
          const end = start + chunkSize;
          const hasMoreText = start < attachedDoc.content.length;

          // Chụp ảnh snapshot của Lorebook và Lịch sử chat trước khi tiến hành bước sinh này nhằm hỗ trợ Rollback/Retry khi có lỗi
          const snapshotLorebookState = JSON.parse(JSON.stringify(currentLorebookState));
          const snapshotHistory = [...currentHistory];

          // Tạo chỉ dẫn loại trừ dập trùng lặp (Strict Deduplication Guard Instruction)
          const deduplicationInstruction = `[BỐI CẢNH QUY TRÌNH HƯỚNG DẪN BẮT BUỘC]
- Các bước bối cảnh ĐÃ HOÀN THÀNH: [${prevStepsMeta || 'Không có, đây là bước đầu tiên'}].
- Nhiệm vụ ĐANG THỰC THI HIỆN TẠI: [Bước ${i + 1}: ${currentStep.name}].
- CHỈ THỊ LOẠI TRỪ DẬP TRÙNG LẶP (DEDUPLICATION EXCLUSION RULE):
  * TUYỆT ĐỐI KHÔNG ĐƯỢC lặp lại, dập trùng hay sinh tiếp bất kỳ mục bối cảnh/lorebook nào đã được mổ xẻ bối cảnh cũ ở các bước trước kia (Ví dụ: Nếu bước trước đã mổ xẻ Thế giới quan rành mạch, tuyệt đối KHÔNG làm tiếp về Thế giới quan nữa ở bước nhân vật này).
  * Bạn chỉ được phép mổ xẻ và dệt thông tin mới tinh khớp chính xác 100% với yêu cầu độc tôn của bước hiện tại: "${currentStep.name}".`;

          // Tạo chỉ thị CHỐNG SKIP GIAI ĐOẠN (Anti-Skip Stage Guard Instruction)
          const antiSkipGuardInstruction = `[CHỈ THỊ CHỐNG SKIP GIAI ĐOẠN]
- Tuyệt đối không được bỏ qua hoặc báo DONE khi chưa trích xuất các thực thể và thông tin cần thiết.
- Phải đảm bảo trích xuất đầy đủ và toàn diện nhất.`;

          if (chunkIndex === 0) {
            currentInput = `[Chạy Quy trình Tự động - ${currentStep.name}]

RULE TỐI CAO ĐƯỢC THIẾT LẬP TRONG HỆ THỐNG:
- Tuyệt đối không được bỏ qua hoặc làm tắt các bước đã quy định. Nếu gặp khó khăn trong việc hiểu yêu cầu của một bước trong Setting, hãy dừng lại và đưa ra câu hỏi làm rõ trước khi tiếp tục.

${antiSkipGuardInstruction}

QUY TRÌNH THU THẬP BẤT BIẾN THEO GIAI ĐOẠN:
1. **Phân tích ngữ cảnh**: Trước khi thu thập, hãy đọc và phân tích toàn bộ 'Các bước hướng dẫn' đã được thiết lập trong phần Setting. Đây là kim chỉ nam cho dữ liệu bạn cần tìm kiếm.
2. **Thực thi theo giai đoạn (Stage-based Execution)**:
   - Giai đoạn hiện tại: Bước ${i + 1}/${enabledSteps.length} - ${currentStep.name}.
   - Chỉ khi hoàn tất giai đoạn trước một cách sạch sẽ, mới được phép chuyển sang giai đoạn tiếp theo.
3. **Tư duy đệ quy & Logic**: So sánh thông tin chính với yêu cầu lọc bối cảnh (chỉ lấy thông tin cốt truyện, nhân vật, bối cảnh, thế giới quan; lọc sạch bình luận tác giả, quản trị viên, fanart, fanfiction, thông tin ngoài lề).
4. **Kiểm tra và Đồng bộ**: Sau mỗi lần thu thập xong một trang con, hãy tự kiểm tra lại dữ liệu đó xem nó đã thỏa mãn các yêu cầu của 'bước hướng dẫn' nào trong Setting hay chưa. Nếu đã đủ, đặt "status": "DONE" để báo hoàn thành bước này và chuyển sang bước tiếp theo.
...
Nội dung bài viết nguồn:
${attachedDoc.content.substring(start, end)}

YÊU CẦU QUAN TRỌNG NHẤT:
1. Con BẮT BUỘC phải đọc và mổ xẻ kĩ lưỡng 100% dữ liệu gốc được cung cấp ở trên.
2. Tự động đề xuất các hành động {"type": "create", "data": {...}} tương ứng để thêm mới hoặc cập nhật thông tin trong Lorebook.
3. Nếu nội dung Wiki quá đồ sộ và con cần viết tiếp, con BẮT BUỘC phải đặt "status": "CONTINUE". Chỉ khi đã trích xuất sạch sẽ 100% không còn sót chi tiết nào của bước này, con mới được đặt "status": "DONE".`;
          } else {
            if (!hasMoreText) {
              if (settings.enableCompletenessProtocol) {
                currentInput = `[Hệ thống] Phân khúc tài liệu Wiki đã kết thúc. Tuy nhiên, theo GIAO THỨC ÉP BUỘC HOÀN THIỆN TỐI ĐA (ZERO OMISSION), con phải đảm bảo trích xuất và tạo Lorebook entry cho TẤT CẢ các thực thể xuất hiện (Bao gồm: Thế giới quan, Hệ thống, Nhân vật, Địa điểm, Sự kiện...).
Không được phép tóm tắt qua loa, rút gọn, hay bỏ sót bất kỳ thông tin chính thức nào.
Nếu vẫn còn thực thể hoặc chi tiết chưa được trích xuất hoàn hảo thành entry, con BẮT BUỘC phải tiếp tục tạo entry và đặt "status": "CONTINUE".
Chỉ khi và chỉ khi không còn sót lại dù chỉ một thực thể hay chi tiết nhỏ nhất nào trong khối dữ liệu hiện tại, con mới được phép kết thúc quy trình và xuất ra mã lệnh "status": "DONE".

${antiSkipGuardInstruction}

${deduplicationInstruction}`;
              } else {
                currentInput = `[Hệ thống] Phân khúc tài liệu Wiki đã kết thúc. Hãy hoàn thiện các phân tích còn sót lại của nhiệm vụ '${currentStep.name}' và trả về "status": "DONE".
...
${deduplicationInstruction}`;
              ...
              currentInput = `[Đọc tiếp tài liệu Wiki - Phân khúc ${chunkIndex}]
...
QUY TRÌNH THU THẬP BẤT BIẾN:
- Đang thực thi Bước ${i + 1}/${enabledSteps.length}: ${currentStep.name} (Phần tải thứ #${chunkIndex + 1})

Nội dung phần tải:
${attachedDoc.content.substring(start, end)}

Hướng dẫn chỉ đạo: ${currentStep.prompt}

${deduplicationInstruction}

Hãy tiếp tục mổ xẻ kĩ lưỡng 100% dữ liệu và tạo Lorebook entries. Nếu còn thông tin để trích lục, hãy đặt "status": "CONTINUE". Nếu đã xong hoàn toàn bước này, hãy đặt "status": "DONE".`;
            }
          }

          const userMsg: ChatMessage = {
            id: `pipeline_u_${Date.now()}_${i}_${chunkIndex}`,
            role: 'user',
            content: `[Hệ thống] Quy trình tự động - Bước ${i + 1}: ${currentStep.name} - Quét mảnh tri thức #${chunkIndex + 1}`,
            timestamp: Date.now(),
            isHidden: true
          };

          const historyToPass = currentHistory.filter(m => m.role !== 'system');
          setStreamBuffer('');
          ...
          try {
            response = await worldbuildingChat(
              currentInput,
              [],
            ...
            consecutiveErrors++;
            setPipelineLogs(prev => [...prev, `[Cảnh báo] API Proxy gặp sự cố: ${err.message}. Lượt lỗi thứ ${consecutiveErrors}/3.`]);
            
            if (consecutiveErrors <= 3) {
              setPipelineLogs(prev => [...prev, `[Tự động Khôi phục] Đang khôi phục snapshot bối cảnh và dệt lại mảnh #${chunkIndex + 1} sau 3 giây...`]);
              currentLorebookState = JSON.parse(JSON.stringify(snapshotLorebookState));
              currentHistory = [...snapshotHistory];
              setMessages([...currentHistory]);
              onApplyActions([]);
              await new Promise(resolve => setTimeout(resolve, 3000));
              continue; // Quay lại đầu vòng lặp chunk, chạy lại mảnh này
            } else {
              setPipelineLogs(prev => [...prev, `[Thất bại] Đã thử dệt lại mảnh #${chunkIndex + 1} quá 3 lần vẫn thất bại. Buộc phải dừng bối cảnh và báo lỗi.`]);
              throw new Error(`Mảnh #${chunkIndex + 1} của Bước ${i + 1} bị lỗi liên tiếp: ${err.message}`);
            }
          }

          if (success && response) {
            // === ANTI-SKIP VALIDATION: Đếm entries thực sự tạo cho bước này ===
            let createdInThisTurn = 0;
            
            if (response.actions && response.actions.length > 0) {
              onApplyActions(response.actions);

              createdInThisTurn = response.actions.filter((a: any) => a.type === 'create').length;
              stepCreatedCount += createdInThisTurn;

              for (const action of response.actions) {
                if (action.type === 'create' && action.data) {
                  const newComment = action.data.comment?.trim().toLowerCase();
                  const exists = currentLorebookState.entries.some(e => e.comment?.trim().toLowerCase() === newComment);
                  if (!exists) {
                    currentLorebookState.entries.push(action.data as any);
                  } else {
                    console.log(`[Local Data Preservation] Entry "${action.data.comment}" already exists. Skipping duplicate.`);
                  }
                } else if (action.type === 'update') {
                  console.log(`[Local Data Preservation] Update action blocked: ${action.target_comment}`);
                } else if (action.type === 'delete') {
                  console.log(`[Local Data Preservation] Delete action blocked: ${action.target_comment}`);
                }
              }

              const createdNames = response.actions
                .filter((a: any) => a.type === 'create')
                .map((a: any) => a.data?.comment)
                .filter(Boolean)
                .join(', ');

              const updatedNames = response.actions
                .filter((a: any) => a.type === 'update')
                .map((a: any) => a.target_comment)
                .filter(Boolean)
                .join(', ');

              if (createdNames) {
                setPipelineLogs(prev => [...prev, `[Tạo mới] Đã hoàn thành mục: ${createdNames}`]);
              }
              if (updatedNames) {
                setPipelineLogs(prev => [...prev, `[Cập nhật] Đã nâng cấp mục: ${updatedNames}`]);
              }
            }

            const assistantMsg: ChatMessage = {
              id: `pipeline_a_${Date.now()}_${i}_${chunkIndex}`,
              role: 'assistant',
            ...
            const readDocActions = response.actions?.filter((a: any) => a.type === 'read_document') || [];
            if (readDocActions.length > 0) {
              chunkIndex = readDocActions[0].chunk_index !== undefined ? readDocActions[0].chunk_index : (chunkIndex + 1);
              consecutiveEmptyDones = 0; // Reset vì đang đọc document
            } else if (response.status === 'CONTINUE') {
              const hasCreatedOrUpdated = response.actions?.some((a: any) => a.type === 'create' || a.type === 'update');
              if (!hasCreatedOrUpdated) {
                chunkIndex++;
              }
              consecutiveEmptyDones = 0; // Reset vì AI biết cần tiếp tục
            } else {
              // === ANTI-SKIP CORE LOGIC: AI trả DONE hoặc status không xác định ===
              
              // KIỂM TRA 1: AI báo DONE nhưng chưa tạo bất kỳ entry nào cho bước này
              if (stepCreatedCount === 0 && consecutiveEmptyDones < MAX_EMPTY_DONE_RETRIES) {
                consecutiveEmptyDones++;
                setPipelineLogs(prev => [...prev, `[⚠️ CHỐNG SKIP] AI báo hoàn thành Bước ${i + 1} nhưng chưa tạo BẤT KỲ entry nào! Đây là lần skip thứ ${consecutiveEmptyDones}/${MAX_EMPTY_DONE_RETRIES}. Ép buộc AI thực hiện lại bước này...`]);
                
                // Inject lệnh ép buộc mạnh vào lịch sử chat 
                const forceRetryMsg: ChatMessage = {
                  id: `pipeline_force_retry_${Date.now()}_${i}_${consecutiveEmptyDones}`,
                  role: 'user',
                  content: `[HỆ THỐNG - LỆNH KHẨN CẤP CHỐNG SKIP GIAI ĐOẠN]

⛔ CẢNH BÁO NGHIÊM TRỌNG: Con vừa báo "DONE" cho Bước ${i + 1} (${currentStep.name}) nhưng CHƯA TẠO BẤT KỲ ENTRY NÀO!
Đây là vi phạm nghiêm trọng quy trình.

LỆNH BẮT BUỘC TUYỆT ĐỐI:
1. Con PHẢI quay lại đọc kỹ tài liệu Wiki đã được cung cấp.
2. Con PHẢI tìm kiếm và trích xuất thông tin ĐÚNG với yêu cầu của Bước ${i + 1}: "${currentStep.name}".
3. Hướng dẫn chỉ đạo: ${currentStep.prompt}
4. Con PHẢI tạo ít nhất 1 entry bằng action {"type": "create", "data": {...}}.
5. TUYỆT ĐỐI KHÔNG ĐƯỢC đặt "status": "DONE" nếu chưa tạo entry nào.
6. Nếu không tìm thấy dữ liệu phù hợp trong chunk hiện tại, hãy đặt "status": "CONTINUE" để quét chunk tiếp theo.

Đây là lần nhắc nhở thứ ${consecutiveEmptyDones}/${MAX_EMPTY_DONE_RETRIES}. Nếu tiếp tục vi phạm, hệ thống sẽ báo lỗi nghiêm trọng.`,
                  timestamp: Date.now(),
                  isHidden: true
                };
                currentHistory.push(forceRetryMsg);
                setMessages([...currentHistory]);
                
                // Tiếp tục vòng lặp, không tăng chunkIndex (quay lại cùng chunk)
                continue;
              }
              
              // KIỂM TRA 2: Đã retry quá nhiều lần mà AI vẫn không tạo entry → Chấp nhận và cảnh báo
              if (stepCreatedCount === 0 && consecutiveEmptyDones >= MAX_EMPTY_DONE_RETRIES) {
                setPipelineLogs(prev => [...prev, `[⚠️ CẢNH BÁO] Bước ${i + 1} (${currentStep.name}) đã retry ${MAX_EMPTY_DONE_RETRIES} lần nhưng AI không tạo được entry nào. Có thể tài liệu không chứa thông tin phù hợp cho bước này. Buộc phải chuyển tiếp.`]);
                stepDone = true;
                const cleanStepName = currentStep.name.replace(/^Bước\s+\d+:\s*/i, '');
                setPipelineLogs(prev => [...prev, `⚠️ Bước [${i + 1}] kết thúc với [0] entry: ${cleanStepName} (KHÔNG CÓ DỮ LIỆU PHÙ HỢP)`]);
              }
              
              // Bước hoàn thành hợp lệ
              else {
                stepDone = true;
                const cleanStepName = currentStep.name.replace(/^Bước\s+\d+:\s*/i, '');
                setPipelineLogs(prev => [...prev, `✅ Đã hoàn thành bước [${i + 1}]: Thu thập được [${stepCreatedCount}] thông tin về [${cleanStepName}]`]);
              }
            }
